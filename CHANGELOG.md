...
- **README example output** — live Modbus TCP dashboard from the 13.25 kW system and docs Q&A about Catch Solar Relay + peak rate configuration
- **Experimental notices** — AppKey/AppSecret auth marked as less tested than user/pass in api-mcp and developer docs
- **Publish on version bump** — `publish.yml` now triggers on push to main and compares local vs npm versions per package, publishing only changed packages (no more GitHub Release dependency)
- **AGENTS.md release process** — documented procedure for cutting releases (detect changes, bump, changelog, commit, tag, push)
